...
<template>
	<details open class="group h-full grow space-y-1 lg:w-1/2">
		<summary
		...
		<transition-group
			...
			:name="`slide-${itemType}`"
			tag="ul"
			class="grid grid-rows-[0fr] divide-y divide-slate-600 rounded-md transition-all duration-700 group-open:grid-rows-[1fr] lg:block">
			<li
				v-for="item in data.allItems[itemType]"
				:id="item.id"
				:key="item.id"
				class="scrollbar-hidden group flex items-center justify-between gap-3.5 overflow-x-auto bg-slate-800 px-3.5 py-2.5 sm:gap-5">
				<!-- Transaction name -->
